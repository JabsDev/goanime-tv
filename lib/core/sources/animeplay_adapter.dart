...
class AnimePlayAdapter implements AnimeSourceAdapter {
  @override
  AnimeSource get source => AnimeSource.animePlay;
  @override
  ...
  Future<ScraperResult<List<Anime>>> search(String query) async {
    return ScraperResult.failure(EmptyResultError(
      message: 'Anime Play search not implemented',
      source: source,
    ));
  }
  ...
  Future<ScraperResult<EpisodesResult>> getEpisodes(Anime anime) async {
    debugPrint('[AnimePlayAdapter] getEpisodes START anime=${anime.name}');
    debugPrint('[AnimePlayAdapter] anime.url=${anime.url}');
    ...
    try {
      ...
      debugPrint('[AnimePlayAdapter] Fetching episodes from: $uri');
      ...
      debugPrint('[AnimePlayAdapter] Response status: ${res.statusCode}');
      ...
      debugPrint('[AnimePlayAdapter] Parsed HTML, looking for episodes');

      // Procurar por elementos de episódio
      final episodeElements = doc.querySelectorAll('.episodio');

      debugPrint('[AnimePlayAdapter] Found ${episodeElements.length} episode elements');
      ...
      debugPrint('[AnimePlayAdapter] getEpisodes ERROR: $e\n$stackTrace');
  ...
  Future<ScraperResult<List<VideoSource>>> getVideoSources(
    ...
    return ScraperResult.failure(EmptyResultError(
      message: 'Anime Play video sources not implemented',
      source: source,
    ));
  }

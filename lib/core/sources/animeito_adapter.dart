...
class AnimeitoAdapter implements AnimeSourceAdapter {
  @override
  AnimeSource get source => AnimeSource.animeIto;
  @override
  ...
  Future<ScraperResult<List<Anime>>> search(String query) async {
    return ScraperResult.failure(EmptyResultError(
      message: 'Anime Ito search not implemented',
      source: source,
    ));
  }
  ...
  Future<ScraperResult<EpisodesResult>> getEpisodes(Anime anime) async {
    debugPrint('[AnimeitoAdapter] getEpisodes START anime=${anime.name}');
    debugPrint('[AnimeitoAdapter] anime.url=${anime.url}');
    ...
    try {
      ...
      debugPrint('[AnimeitoAdapter] Fetching episodes from: $uri');
      ...
      debugPrint('[AnimeitoAdapter] Response status: ${res.statusCode}');
      ...
      debugPrint('[AnimeitoAdapter] Parsed HTML, looking for episodes');

      // Procurar por elementos de episódio
      final episodeElements = doc.querySelectorAll('.eps1');

      debugPrint('[AnimeitoAdapter] Found ${episodeElements.length} episode elements');
      ...
      debugPrint('[AnimeitoAdapter] getEpisodes ERROR: $e\n$stackTrace');
  ...
  Future<ScraperResult<List<VideoSource>>> getVideoSources(
    ...
    return ScraperResult.failure(EmptyResultError(
      message: 'Anime Ito video sources not implemented',
      source: source,
    ));
  }

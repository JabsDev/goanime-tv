...
class AnimeQAdapter implements AnimeSourceAdapter {
  @override
  AnimeSource get source => AnimeSource.animeQ;
  @override
  ...
  Future<ScraperResult<List<Anime>>> search(String query) async {
    return ScraperResult.failure(EmptyResultError(
      message: 'Anime Q search not implemented',
      source: source,
    ));
  }
  ...
  Future<ScraperResult<EpisodesResult>> getEpisodes(Anime anime) async {
    debugPrint('[AnimeQAdapter] getEpisodes START anime=${anime.name}');
    debugPrint('[AnimeQAdapter] anime.url=${anime.url}');
    ...
    try {
      ...
      debugPrint('[AnimeQAdapter] Fetching episodes from: $uri');
      ...
      debugPrint('[AnimeQAdapter] Response status: ${res.statusCode}');
      ...
      debugPrint('[AnimeQAdapter] Parsed HTML, looking for episodes');

      // Procurar por elementos de episódio
      final episodeElements = doc.querySelectorAll('.eps1');

      debugPrint('[AnimeQAdapter] Found ${episodeElements.length} episode elements');
      ...
      debugPrint('[AnimeQAdapter] getEpisodes ERROR: $e\n$stackTrace');
  ...
  Future<ScraperResult<List<VideoSource>>> getVideoSources(
    ...
    return ScraperResult.failure(EmptyResultError(
      message: 'Anime Q video sources not implemented',
      source: source,
    ));
  }
